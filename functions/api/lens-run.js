...
async function callGemini(env,prompt,web){if(!env.GEMINI_API_KEY)throw new Error('GEMINI_API_KEY not configured');const model=env.GEMINI_MODEL||'gemini-2.5-pro';const body={contents:[{parts:[{text:prompt}]}]};if(web)body.tools=[{google_search:{}}];const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(env.GEMINI_API_KEY)}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const j=await r.json();if(!r.ok)throw new Error(j.error?.message||'Gemini HTTP '+r.status);return j.candidates?.[0]?.content?.parts?.map(x=>x.text||'').join('\n')||'NO_TEXT_OUTPUT'}
async function callPerplexity(env,prompt,web){if(!env.PERPLEXITY_API_KEY)throw new Error('PERPLEXITY_API_KEY not configured');const r=await fetch('https://api.perplexity.ai/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${env.PERPLEXITY_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:env.PERPLEXITY_MODEL||'sonar-pro',messages:[{role:'user',content:prompt}]})});const j=await r.json();if(!r.ok)throw new Error(j.error?.message||'Perplexity HTTP '+r.status);return j.choices?.[0]?.message?.content||'NO_TEXT_OUTPUT'}

async function analyzeFrozen(env,frozen){const prompt=`You are the AI-LENS frozen-chain normalizer. NO WEB SEARCH. Use ONLY the JSON evidence below. Never diagnose technical root cause. Never prescribe remediation. Produce STRICT JSON with keys p4,p5,p6,p7, each a concise DISPLAY-style multiline string. P4 compares controlled P1/P2 against P3 and states FULL/DEGRADED/LOST/NOT_MEASURED. P5 classifies ACCESS/RETRIEVAL/UNDERSTANDING/SELECTION/USAGE gaps only when supported. P6 normalizes one view per AI preserving provenance. P7 merges cross-AI agreement/divergence and explicitly states whether there is a single AI view. Evidence:\n${JSON.stringify(frozen).slice(0,120000)}`;try{const raw=await callOpenAI(env,prompt,false);const m=raw.match(/\{[\s\S]*\}/);if(m)return JSON.parse(m[0])}catch{}return null}
